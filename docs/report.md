...
This project assumes a standard web threat model in which attackers may obtain user passwords through phishing, credential reuse, or database leaks. The system is designed to mitigate the impact of such compromises by requiring a second authentication factor based on Time-Based One-Time Passwords (TOTP).

The TOTP mechanism assumes that the attacker does not have physical or logical access to the user’s authenticator device. Even if the attacker successfully logs in using valid credentials, access is denied unless a valid time-based code is provided.

Replay attacks against the TOTP mechanism are considered within an authenticated session context. Since TOTP codes are time-limited and expire every 30 seconds, previously valid codes cannot be reused successfully, providing protection against replay attacks during the second authentication phase.
...
The session explicitly tracks whether the user has completed the second authentication factor, preventing access to protected routes until both authentication steps are successfully completed.
...
Replay attacks target the reuse of previously valid authentication data. In the context of TOTP-based 2FA, replay attacks assume that the attacker has already passed the primary authentication step (username and password) and attempts to reuse an old one-time password.

Due to the time-based nature of TOTP, codes are only valid for a short window. Any previously captured or expired code is rejected by the verification process, effectively preventing replay attacks during the second authentication phase.
...
All demonstrations were performed on test accounts and did not involve real user data.
